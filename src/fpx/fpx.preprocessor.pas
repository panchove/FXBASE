...
uses
  SysUtils, fpx.tokens, fpx.errors;
...
type
  ...
  private
    ...
    FLine: Integer;
    FOutput: TFPXStringList;
    FIncludePaths: TFPXStringList;
  ...
  public
    ...
    property Defines: TFPXStringList read FDefines;
    property Output: TFPXStringList read FOutput;
  end;
...
constructor TFPXStringList.Create;
begin
  inherited Create;
  FStrings := nil;
  FText := '';
...
destructor TFPXStringList.Destroy;
begin
  FStrings := nil;
  inherited Destroy;
end;

procedure TFPXStringList.Add(const s: string);
begin
  SetLength(FStrings, Length(FStrings) + 1);
  FStrings[High(FStrings)] := s;
end;

procedure TFPXStringList.AddPair(const name, value: string);
begin
  SetLength(FStrings, Length(FStrings) + 1);
  FStrings[High(FStrings)] := name + '=' + value;
end;

function TFPXStringList.IndexOfName(const name: string): Integer;
var
  i: Integer;
  s: string;
begin
...
procedure TFPXStringList.Delete(index: Integer);
var
  i: Integer;
begin
...
function TFPXStringList.Count: Integer;
begin
  Result := Length(FStrings);
end;

function TFPXStringList.GetName(index: Integer): string;
var
  s: string;
  p: Integer;
begin
...
procedure TFPXStringList.SetValue(const name, value: string);
var
  i: Integer;
begin
  i := IndexOfName(name);
...
function TFPXStringList.GetItems(index: Integer): string;
begin
  if (index < 0) or (index > High(FStrings)) then Exit('');
  Result := FStrings[index];
end;

procedure TFPXStringList.SetItems(index: Integer; const value: string);
begin
  if (index < 0) or (index > High(FStrings)) then Exit;
  FStrings[index] := value;
end;

procedure TFPXStringList.Clear;
begin
  SetLength(FStrings, 0);
  FText := '';
end;

function TFPXStringList.ValueFromIndex(index: Integer): string;
var
  s: string;
  p: Integer;
begin
...
begin
  inherited Create;
  FReporter := Reporter;
  FDefines := TFPXStringList.Create;
  FDefines.AddPair('__FPXBASE__', '1');
  FDefines.AddPair('__FPXBASE_VER__', '100');
  FDefines.AddPair('__LINE__', '0');
  FDefines.AddPair('__FILE__', '');
  FMacros := nil;
  FCommands := nil;
  FIncludes := TFPXStringList.Create;
  FIncludePaths := TFPXStringList.Create;
  FOutput := TFPXStringList.Create;
  FActive := True;
end;
...
begin
  ...
  if fn = '' then
  begin
    FReporter.ErrorFPX(FPX_INCLUDE_NESTING, 'Empty #include', FLine, 1);
    Exit;
  end;
  ...
  begin
    FReporter.ErrorFPX(FPX_INCLUDE_NESTING, 'Include file not found: ' + fn, FLine, 1);
    Exit;
  end;

  sl := TFPXStringList.Create;
  try
...
begin
  ...
  begin
    FReporter.ErrorFPX(FPX_INVALID_DIRECTIVE, '#else without #ifdef', FLine, 1);
    Exit;
  end;
  ...
  begin
    if ElseSeen then
    begin
      FReporter.ErrorFPX(FPX_INVALID_DIRECTIVE, 'Duplicate #else', FLine, 1);
      Exit;
    end;
    ElseSeen := True;
...
begin
  if Length(FIfdefStack) = 0 then
  begin
    FReporter.ErrorFPX(FPX_INVALID_DIRECTIVE, '#endif without #ifdef', FLine, 1);
    Exit;
  end;
  with FIfdefStack[High(FIfdefStack)] do
    FActive := Active;
...
begin
  FReporter.ErrorFPX(FPX_SYNTAX_ERROR, '#error: ' + Arg, FLine, 1);
end;
...
procedure TFPXStringList.LoadFromFile(const filename: string);
var
  f: TextFile;
  line: string;
begin
...
procedure TFPXStringList.SaveToFile(const filename: string);
var
  f: TextFile;
  line: string;
begin
...
function TFPXStringList.GetText: string;
begin
  Result := FText;
end;

procedure TFPXStringList.SetText(const value: string);
var
