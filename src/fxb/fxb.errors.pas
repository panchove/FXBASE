...
type
  ...
  public
    ...
    procedure ErrorFXB(Code: Integer; const Text: string; Line, Col: Integer);
...
// FXB error codes (FXB-nnnn)
const
  FXB_SYNTAX_ERROR     = 1;
  FXB_UNEXPECTED_TOKEN = 2;
  FXB_UNTERMINATED_STR = 3;
  FXB_INVALID_CHAR     = 4;
  FXB_EXPECTED_IDENT   = 5;
  FXB_EXPECTED_EXPR    = 6;
  FXB_UNDEFINED_VAR    = 7;
  FXB_TYPE_MISMATCH    = 8;
  FXB_DUPLICATE_DEF    = 9;
  FXB_INVALID_OP       = 10;
  FXB_UNTERMINATED_BLOCK = 11;
  FXB_INVALID_DIRECTIVE = 12;
  FXB_MACRO_RECURSION  = 13;
  FXB_INCLUDE_NESTING  = 14;
  FXB_NO_ENTRY_POINT   = 15;
  FXB_UNSUPPORTED_FEATURE = 16;

  FXB_DESCRIPTIONS: array[1..16] of string = (
    'Syntax error',
    'Unexpected token',
...
procedure TErrorReporter.ErrorFXB(Code: Integer; const Text: string;
  Line, Col: Integer);
var
  codeStr: string;
  desc: string;
begin
  if (Code >= Low(FXB_DESCRIPTIONS)) and (Code <= High(FXB_DESCRIPTIONS)) then
    desc := FXB_DESCRIPTIONS[Code]
  else
    desc := 'Unknown error';
  Str(Code, codeStr);
