...
uses
  sysutils, classes, fxb.tokens, fxb.errors, fxb.ast, fxb.lexer, fxb.parser.types;
...
begin
  case Peek of
    ...
    begin
      Error(FXB_EXPECTED_EXPR, 'Expected expression, got ' + DumpToken(FCurrent));
      Result := TLiteralExpr.Create(FCurrent);
      Advance;
    end;
  end;
...
begin
  ...
  repeat
    ...
    begin
      ...
      Error(FXB_EXPECTED_IDENT, 'Expected parameter name');
  until Peek = ttRParen;
...
begin
  ...
  begin
    Error(FXB_EXPECTED_IDENT, 'Expected function name');
    name := '__missing__';
  end;
  Advance;
  ...
  if CheckKeyword(kwEndFunc) or CheckKeyword(kwEndFunction) then
    ...
    Error(FXB_UNTERMINATED_BLOCK, 'Expected ENDFUNC to close FUNCTION');
...
begin
  ...
  begin
    Error(FXB_EXPECTED_IDENT, 'Expected procedure name');
    name := '__missing__';
  end;
  Advance;
  ...
  if CheckKeyword(kwEndProc) or CheckKeyword(kwEndProcedure) then
    ...
    Error(FXB_UNTERMINATED_BLOCK, 'Expected ENDPROC to close PROCEDURE');
...
begin
  ...
  begin
    Error(FXB_EXPECTED_IDENT, 'Expected class name');
    name := '__missing__';
  end;
...
begin
  ...
  begin
    Error(FXB_EXPECTED_IDENT, 'Expected struct name');
    name := '__missing__';
  end;
...
begin
  ...
  begin
    Error(FXB_EXPECTED_IDENT, 'Expected type name');
    name := '__missing__';
  end;
