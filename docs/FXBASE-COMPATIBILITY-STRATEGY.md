# FXBASE — Estrategia de Compatibilidad xBase (Tiered 80/20)
...
El ecosistema xBASE tiene ~40 años de historia y dialectos divergentes (Clipper 5.x, Harbour, FoxPro 2.x, Visual FoxPro). Una reimplementación 1:1 arrastraría décadas de deuda técnica, comportamientos implícitos no documentados y dependencias de un motor RDD binario que **FXBASE ha decidido reemplazar por SQL**.
...
Cada tier es **opt-in progresivo** (compilación por archivo / bloque) — el código nuevo FXBASE no paga el costo de compatibilidad si no lo usa.
...
En xBASE clásico: `DECLARE` admitía `DECL`, `PROCEDURE` admitía `PROC`, `FUNCTION` admitía `FUNCT`, etc. **FXBASE exige la palabra completa.** Razón: el lexer case-insensitive colapsaría 4× variantes por keyword, inflando la `FKeywordMap` y creando falsos positivos (`LOCAL` vs `LOCATE`).

```xbase
PROCEDURE Main()              ; OK
PROC Main()                   ; FX-LE-0007 — keyword truncada
```
...
- **Comandos con semántica completa en IR:** `IF/ELSE/ENDIF`, `DO WHILE/ENDDO`, `FOR/NEXT`, `?`, `??`. (Phase 1.1 — `fx.ir.pas` `TFXIRGenerator` ya baja los tres primeros.)
- **Comandos pendientes de IR:** `@…SAY`, `@…GET` (requieren `fx.rtl` no-stub).
...
FXBASE **NO abre `.dbf/.cdx/.ntx/.fpt` como estado runtime**. Solo `fx-dbf` (herramienta CLI planificada, sin código aún) puede **importar** schema + datos a SQLite/Postgres/MSSQL o **exportar** una tabla SQL a `.dbf` para interoperabilidad con sistemas externos.
...
LOCAL op := "+"
? 10 &op 5             ; FX-LE-0010 — operador dinámico (warning en compile, error en #strict)
```
...
### 5.1 `fx-dbf` — Importador / Exportador

CLI que migra datos y schema:

```bash
fx-dbf import  customers.dbf  --into sqlite://data/app.db
fx-dbf export  customers      --from sqlite://data/app.db  --to customers.dbf --format "dBASE IV"
fx-dbf schema  customers.dbf  --emit-sql sqlite
```
...
| T2   | Apps con DBF                | Medio: ejecutar `fx-dbf import` + ajustar sintaxis de PACK/ZAP |
