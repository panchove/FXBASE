...
## 🏗️ Arquitectura General del Compilador FXBASE (Orientada a Velocidad)
...
```mermaid
flowchart TD
    subgraph COMPILADOR["COMPILADOR FXBASE"]
        direction TB
        subgraph Workers
...
> **Estado:** Roadmap — pendiente de implementación. La capa RDD virtual (`USE`/`SKIP`/`SEEK` traducidos a SQL) vive en `fx.rtl.pas`, que es stub. La estrategia de prefetching que se documenta aquí minimiza el clásico **problema N+1** en bucles legacy traducidos a SQL.
