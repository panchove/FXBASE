...
|------------------------------------------|-----------------------------------------------------------------------------------|
| Archivos `.dbf` en runtime               | Toda persistencia es SQL; `.dbf` solo vía `fpx-dbf` import/export                 |
...
| `DIR`, `DISPLAY STRUCTURE`, `LIST`       | Reemplazado por comandos del shell o `fpx` CLI                                    |
...
- Herramienta `fpx-dbf` para **importar** archivos `.dbf` (estructura + datos + índices) a SQLite/PostgreSQL/SQL Server.
- Herramienta `fpx-dbf` para **exportar** tablas SQL a formato `.dbf` compatible con Clipper/Harbour/FoxPro.
...
|---------------------------------|--------------------------------------------------------------|------------------------------------|
| DBF (dBASE III, IV, FoxPro 2.x) | Archivo `.dbf` + `.fpt`/`.dbt` (memo)                        | `fpx-dbf import`                   |
...
|---------|-------------------------------------------|-----------------------------------------------|
| DBF     | Archivo `.dbf` (dBASE III/IV, FoxPro 2.x) | `fpx-dbf export`                              |
...
- **`fpx-dap`** — servidor Debug Adapter Protocol para integración con VS Code, Emacs, etc.
...
- `fpx build --release` → compila con `-O2` sin debug info
- `fpx build --dist msi` → genera MSI (Windows)
- `fpx build --dist appimage` → genera AppImage (Linux)
- `fpx build --dist zip` → genera archivo comprimido portable
- `fpx deploy --server user@host:/path` → sube binario y reinicia servicio
...
- **fpx** — CLI del compilador (build, run, test).

  Parámetros del compilador `fpx`:
...
- **fpx-lsp** — Language Server Protocol (diagnósticos, completado, ir a definición).
- **fpx-fmt** — Formateador de código.
- **fpx-pkg** — Gestor de paquetes (p. ej. `fpx install csv-lib`).
...
1. El usuario importa sus .dbf a SQLite con `fpx-dbf import --input datos.dbf --output data.db`.
2. El compilador compila el código Harbour con `fpx build --db sqlite --connection "data.db"`.
3. Una vez validado, se cambia la cadena de conexión a PostgreSQL o SQL Server para producción.
4. Si es necesario, se exportan los datos de vuelta a .dbf con `fpx-dbf export --input data.db --output datos.dbf`.
...
Compilado con `fpx build --db sqlite --connection "data.db"` generaría equivalente a:
...
3. **Migración:** Herramienta `fpx-dbf` capaz de migrar una base .dbf completa (estructura + datos) a SQLite/PG/MSSQL en **menos de 60 segundos para 1 GB de datos**.
...
| **5** | Herramienta `fpx-dbf` (import/export DBF), modo legacy, documentación         | 2 meses           |
